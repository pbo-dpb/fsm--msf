...
export class Capability {
    constructor(payload) {
        ...
        this.environment = environmentStore.environments.find(
            (env) => env.id === payload.environment_id,
        );
        ...
        this.user_editable = userEditableRange
            ? {
                  range_min:
                      this.current - userEditableRange >= 0
                          ? this.current - userEditableRange
                          : 0,
                  range_max: this.current + userEditableRange,
                  step: payload.user_editable_step
                      ? payload.user_editable_step
                      : 1,
    ...
    get userTargetImpact() {
        ...
        for (const [aspect, aspectFacets] of Object.entries(
            this.impactPerUnit,
        )) {
            if (!impact[aspect]) {
                impact[aspect] = {};
            ...
            for (const [facet, impactForFacet] of Object.entries(
                ...
                let value =
                    this[aspect][facet] + userTargetDiff * impactForFacet;
                let roundedNum =
                    aspect === "cost"
