<template>
    <tr :class="{ 'bg-purple-50 dark:bg-purple-950': highlight }">
        ...
        <th
            scope="row"
            class="py-.5 border border-solid border-gray-300 px-1 font-normal dark:border-gray-900"
        ...
        <td
            ...
            class="py-.5 border border-solid border-gray-300 px-1 text-center tabular-nums dark:border-gray-900"
            :class="[shouldDisplayImpactDiff ? 'w-1/4' : 'w-1/3']">
        ...
        <template v-if="shouldDisplayImpactDiff">
            <td
                class="py-.5 w-1/4 border border-solid border-gray-300 px-1 text-center tabular-nums dark:border-gray-900"
                :class="[
                    highlight
...
<script setup>
    ...
    watch(hasValidCapability, (isValid) => {
        if (!isValid) {
            console.warn(
                "Compliance capability not found in capabilities list",
            );
        }
    });
</script>
