...
<script setup>
    ...
    function getValForFacet(facet, userTarget = false) {
        if (settings_perUnitDisplay.value) {
            return formatValue(
                props.capability.impactPerUnit[props.aspect][facet],
            );
        }
        return formatValue(
            userTarget
                ? props.capability.userTargetImpact[props.aspect][facet]
                : props.capability[props.aspect][facet],
        );
    }
</script>
