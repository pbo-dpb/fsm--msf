...
<script setup>
    ...
    const generalOverhead = computed(() => {
        ...
        return capabilities.value.reduce((accumulator, capability) => {
            return (
                accumulator +
                capability.userTargetImpact[settings_selectedAspect.value]
                    .inst_overhead
            );
        }, 0);
    });
    ...
    const environmentOverheads = computed(() => {
        ...
        Object.entries(groupedCapabilities.value || {}).forEach(
            ([envId, gr]) => {
                ...
                let label =
                    strings.value.per_environment_env_overhead_label.replace(
                        ":environment",
        ...
        datasets: [
            {
                ...
                backgroundColor: [
                    colorForIndex(2),
                    ...Object.keys(environmentOverheads.value).map(() =>
                        colorForIndex(3),
                    ),
                ],
            },
