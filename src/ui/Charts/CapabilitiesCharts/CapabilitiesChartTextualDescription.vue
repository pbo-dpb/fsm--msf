<template>
    <details class="flex flex-col gap-2 print:hidden">
        <summary
        ...
        <div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr
                            v-for="(
                                label, index
                            ) in environmentCapabilitiesChartData.labels"
                            :key="label"
            ...
            <dl
                class="mt-2 border-t-2 border-solid border-gray-300 pt-2 dark:border-gray-700">
                <div
...
<script setup>
    ...
    const props = defineProps({
        ...
        environmentOverheadChartData: {
            ...
            required: true,
            validator: (value) => {
                return value.datasets && Array.isArray(value.datasets);
            },
        },
    });
    ...
    const isDataValid = computed(() => {
        return props.environmentCapabilitiesChartData.datasets.every(
            (dataset) =>
                dataset.data.length ===
                props.environmentCapabilitiesChartData.labels.length,
        );
    });
